    ...
    <main className="min-h-screen" style={{ background: 'linear-gradient(135deg, #fdf2f8 0%, #fce7f3 40%, #f5f0ff 100%)' }}>
      ...
      <nav style={{ background: 'rgba(253,242,248,0.9)', backdropFilter: 'blur(12px)', borderBottom: '1px solid rgba(190,24,93,0.1)', position: 'sticky', top: 0, zIndex: 50 }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '14px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <span style={{ fontSize: '1.25rem', fontWeight: 700, color: '#be185d', fontFamily: 'Georgia, serif' }}>{t('appName')}</span>
          </div>
          <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            ...
            <SignedIn>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <Link href="/dashboard">
                  <button className="btn-primary" style={{ padding: '9px 18px', fontSize: '0.82rem' }}>{t('dashboard')}</button>
                </Link>
            ...
            <SignedOut>
              <Link href="/sign-in">
                <button className="btn-secondary" style={{ padding: '9px 16px', fontSize: '0.82rem' }}>{t('signIn')}</button>
              </Link>
              <Link href="/sign-up">
                <button className="btn-primary" style={{ padding: '9px 16px', fontSize: '0.82rem' }}>{t('signUp')}</button>
              </Link>
            </SignedOut>
          </div>
        </div>
      ...
      <section style={{ maxWidth: 1200, margin: '0 auto', padding: 'clamp(40px,8vw,80px) 16px clamp(30px,5vw,60px)', textAlign: 'center' }}>
        ...
        <h1 style={{ fontSize: 'clamp(2rem, 7vw, 4.5rem)', fontFamily: 'Georgia, serif', fontWeight: 700, color: '#1a0a12', lineHeight: 1.15, margin: '0 auto 20px', maxWidth: 800 }}>
          {t('heroTitle1')}<br />
          <span style={{ color: '#be185d' }}>{t('heroTitle2')}</span>
        </h1>
        ...
        <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap', padding: '0 16px' }}>
          <button className="btn-primary" onClick={() => navigate('/sos')} style={{ fontSize: 'clamp(0.85rem, 2vw, 1rem)', padding: 'clamp(12px,2vw,15px) clamp(24px,4vw,36px)' }}>
            {t('getHelpDiscreetly')}
          </button>
          <button className="btn-secondary" onClick={() => navigate('/authority')} style={{ fontSize: 'clamp(0.85rem, 2vw, 1rem)', padding: 'clamp(12px,2vw,15px) clamp(24px,4vw,36px)' }}>
            {t('authorityDashboard')}
          </button>
        </div>

        <p style={{ marginTop: 16, fontSize: '0.8rem', color: '#be185d', padding: '0 16px' }}>
          {t('emergencyCall')} <strong>112</strong> | {t('womenHelpline')} <strong>181</strong>
        </p>
      </section>
      ...
      <section style={{ maxWidth: 1200, margin: '0 auto', padding: '20px 16px clamp(40px,6vw,80px)' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 260px), 1fr))', gap: 16 }}>
          {[
            ...
            <div key={i} onClick={() => navigate(f.href)} className="glass-card"
              style={{ padding: 'clamp(20px,4vw,32px)', cursor: 'pointer', transition: 'all 0.3s', background: f.color, height: '100%' }}
            ...
            >
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 16, color: '#be185d', fontWeight: 600, fontSize: '0.82rem' }}>
                <span>{t('learnMore')}</span><ChevronRight size={13} />
              </div>
            </div>
          ))}
        </div>
      ...
      <footer style={{ padding: '24px 16px', textAlign: 'center', borderTop: '1px solid rgba(190,24,93,0.1)' }}>
        <p style={{ color: '#8b6b7d', fontSize: 'clamp(0.75rem, 2vw, 0.85rem)' }}>{t('footerText')}</p>
        <p style={{ color: '#be185d', fontSize: 'clamp(0.72rem, 1.8vw, 0.82rem)', marginTop: 6 }}>{t('emergencyFooter')}</p>
      </footer>
    </main>
  )
}