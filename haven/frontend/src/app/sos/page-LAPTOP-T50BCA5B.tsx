    ...
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #fdf2f8 0%, #fce7f3 100%)' }}>
      ...
      <div style={{ background: 'rgba(253,242,248,0.9)', backdropFilter: 'blur(12px)', borderBottom: '1px solid rgba(190,24,93,0.1)', padding: '14px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <Link href="/home">
            <button style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '8px', display: 'flex', alignItems: 'center', color: '#be185d' }} aria-label="Go back">
              <ArrowLeft size={18} />
            </button>
          </Link>
      ...
      <div style={{ maxWidth: 680, margin: '0 auto', padding: 'clamp(20px,4vw,40px) 16px' }}>
        ...
        <div style={{ background: '#fee2e2', border: '1px solid #fca5a5', borderRadius: 12, padding: '12px 16px', marginBottom: 24, display: 'flex', gap: 10, alignItems: 'flex-start' }}>
          ...
          <p style={{ fontSize: 'clamp(0.78rem, 2.5vw, 0.85rem)', color: '#dc2626', lineHeight: 1.5 }}>
            {t('inImmediateDangerSOS')} <strong>112</strong> or Women's Helpline <strong>181</strong>
          </p>
        </div>
        ...
        <div className="glass-card" style={{ padding: 'clamp(20px,5vw,36px)' }}>
          {step === 'message' && (
            <div>
              <h2 style={{ fontSize: 'clamp(1.15rem, 4vw, 1.4rem)', fontFamily: 'Georgia', marginBottom: 8, color: '#1a0a12' }}>{t('describeSituation')}</h2>
              ...
              <button className="btn-primary" onClick={expandMessage} disabled={loading || !keywords.trim()} style={{ marginTop: 16, width: '100%', opacity: loading || !keywords.trim() ? 0.6 : 1 }}>
                {loading ? t('expanding') : t('createDistressMsg')}
              </button>
            </div>
          )}
          ...
          {step === 'share' && (
            <div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 8, marginBottom: 10 }}>
                ...
                <button onClick={() => window.open(`https://twitter.com/intent/tweet?text=${encodeURIComponent('I need help. #HavenSOS #WomenSafety')}`, '_blank')} style={{ padding: '11px 10px', borderRadius: 12, border: 'none', background: '#000', color: 'white', fontWeight: 600, fontSize: 'clamp(0.75rem, 2vw, 0.85rem)', cursor: 'pointer' }}>Twitter/X</button>
                ...
                <button onClick={() => window.open(`https://t.me/share/url?url=https://haven.app&text=${encodeURIComponent('I need help. #HavenSOS')}`, '_blank')} style={{ padding: '11px 10px', borderRadius: 12, border: 'none', background: '#0088cc', color: 'white', fontWeight: 600, fontSize: 'clamp(0.75rem, 2vw, 0.85rem)', cursor: 'pointer' }}>Telegram</button>
